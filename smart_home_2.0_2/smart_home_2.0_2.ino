...
int           delayPeriodRGB    = 1000;
int           countTimeRGB      = 0;

unsigned long startTimeCooler   = 0;
int           delayPeriodCooler = 2000;
...
void loop()
{
  ...
  {
    unsigned long currentTimeCooler = millis();

    if (currentTimeCooler - startTimeCooler > delayPeriodCooler)
    {
      startTimeCooler = currentTimeCooler;
...
void flashRGB()
{
  unsigned long currentTimeRGB = millis();

  if (currentTimeRGB - startTimeRGB > delayPeriodRGB)
  {
    startTimeRGB = currentTimeRGB;

    digitalWrite(PIN_RGB_RED,   RED_BLINK  [countTimeRGB]);
    digitalWrite(PIN_RGB_GREEN, GREEN_BLINK[countTimeRGB]);
    digitalWrite(PIN_RGB_BLUE,  BLUE_BLINK [countTimeRGB]);
  }
  countTimeRGB++;

  if (countTimeRGB >= 7) countTimeRGB = 0;
}
