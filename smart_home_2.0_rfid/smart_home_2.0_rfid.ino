...
#define PIN_BT_LED_2            53
#define PIN_FOTO_R_LASER        A10          //фоторезистор
...
boolean       signalisationState = false;
...
void loop()
{
  ...
  static int COOLER_ON_BARRIER  = getTemperature();
 ...
 if (success)
  {
    if (signalisationState == false)
    {
      turn_on = true;
    }
    else if (signalisationState == true)
    {
      turn_on = false;
    }
    signalisationState = !signalisationState;
  }
...
if (turn_on == true)
  ...
  if (turn_on == true)
  {
    if (fotoRLaserVal > FOTO_R_LASER_BARRIER)  //если показания с фоторезистора ниже 900 и кнопка на телефоне нажата то...
    {
